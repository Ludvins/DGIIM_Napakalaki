...
class CardDealer
  ...
  def initMonsterDeck
    ...
    p = Prize.new(2, 1)
    b = SpecificBadConsequence.new('Pierdes tu armadura visible y otra oculta', 0, [TreasureKind::ARMOR], [TreasureKind::ARMOR])
    @unusedMonsters << Monster.newStandard('3 byakhees de bonanza', 8, p, b)
    
    # Tenochtitlan
    p = Prize.new(1, 1)
    b = SpecificBadConsequence.new('Embobados con el primigenio te descartas de tu casco visible.', 0, [] , [TreasureKind::HELMET])
    @unusedMonsters << Monster.newStandard('Tenochtitlan', 2, p, b)
    
    # El sopor de Dunwich
    p = Prize.new(1, 1)
    b = SpecificBadConsequence.new('El primordial bostezo contagioso, pierdes el calzado visible', 0, [TreasureKind::SHOES], [])
    @unusedMonsters << Monster.newStandard('El sopor de Dunwich', 2, p, b)
    
    # Demonios de magaluf
    p = Prize.new(4, 1)
    b = SpecificBadConsequence.new("Te atrapan para llevarte de fiesta y te dejan caer en mitad de vuelo. Descarta 1 mano visible y 1 mano oculta.", 0, [TreasureKind::ONEHAND], [TreasureKind::ONEHAND])
    @unusedMonsters << Monster.newStandard('Demonios de magaluf', 2, p, b)
    
    # El gorron en el umbral
    p = Prize.new(3, 1)
    b = NumericBadConsequence.new("pierdes todos tus tesoros visibles", 0, 10, 0)
    @unusedMonsters << Monster.newStandard('El gorron del umbral', 13, p, b)
    
    # H.P munchcraft
    p = Prize.new(2, 1)
    b = SpecificBadConsequence.new("Pierdes la armadura visible", 0, [TreasureKind::ARMOR], [])
    @unusedMonsters << Monster.newStandard('H.P. Munchcraft', 6, p, b)
    
    # Necrofago
    p = Prize.new(1, 1)
    b = SpecificBadConsequence.new("Sientes bichos bajo la ropa. Descarta la armadura visible", 0, [TreasureKind::ARMOR], [])
    @unusedMonsters << Monster.newStandard('Necrofago', 13, p, b)
    
    # El rey rosado
    p = Prize.new(3,2)
    b = NumericBadConsequence.new('Pierdes 5 niveles y 3 tesoros visibles',5 , 3, 0)
    @unusedMonsters << Monster.newStandard('El rey rosado',11, p, b)
    
    # Flecher
    p = Prize.new(1, 1)
    b = NumericBadConsequence.new("Toses los pulmones y pierdes 2 niveles", 2, 0, 0)
    @unusedMonsters << Monster.newStandard('Flecher', 2, p, b)
    
    # los hondos
    p = Prize.new(2, 1)
    b = DeathBadConsequence.new('Estos monstruos resultan bastante superficiales y te aburren mortalmente. Estas muerto')
    @unusedMonsters << Monster.newStandard('Los hondos', 8, p, b)
    
    # semillas cthulhu
    p = Prize.new(2, 1)
    b = NumericBadConsequence.new('Pierdes 2 niveles y 2 tesoros ocultos', 2, 0, 2)
    @unusedMonsters << Monster.newStandard('Semillas Cthulhu', 4, p, b)
    
    # Dameargo
    p = Prize.new(2, 1)
    b = SpecificBadConsequence.new('Te intentas escaquear. Pierdes una mano visible', 0, [TreasureKind::ONEHAND], [])
    @unusedMonsters << Monster.newStandard('Dameargo', 1, p, b)
    
    # Pollipolipo volante
    p = Prize.new(2, 1)
    b = NumericBadConsequence.new('Da mucho asquito. Pierdes 3 niveles', 3, 0, 0)
    @unusedMonsters << Monster.newStandard('Pollipolipo volante', 3, p, b)
    
    # Yskhtihyssg-Goth
    p = Prize.new(3, 1)
    b = DeathBadConsequence.new('No le hace gracia que pronuncien mal su nombre. Estas muerto')
    @unusedMonsters << Monster.newStandard('Yskhtihyssg-Goth', 14, p, b)
    
    # Familia Feliz
    p = Prize.new(3, 1)
    b = DeathBadConsequence.new('La familia te atrapa. Estas muerto')
    @unusedMonsters << Monster.newStandard('Familia feliz', 1, p, b)
    
    # Roboggoth
    p = Prize.new(2, 1)
    b = SpecificBadConsequence.new('La quinta directiva primitiva te obliga a perder 2 niveles y un tesoro 2 manos visible',2,[TreasureKind::BOTHHANDS],[])
    @unusedMonsters << Monster.newStandard('Robbogoth', 8, p, b)
    
    # El Espía sordo
    p = Prize.new(1, 1)
    b = SpecificBadConsequence.new('Te asusta en la noche.Pierdes un casco visible',0,[TreasureKind::HELMET],[])
    @unusedMonsters << Monster.newStandard('El espia sordo', 5, p, b)
    
    #Tongue
    p = Prize.new(2, 1)
    b = NumericBadConsequence.new('Menudo susto te llevas. Pierdes 2 niveles y 5 tesoros visibles.', 2, 5, 0)
    @unusedMonsters << Monster.newStandard('Tongue', 19, p, b)
    
    #Bicéfalo
    p = Prize.new(2, 1)
    b = SpecificBadConsequence.new('Te faltan manos para tanta cabeza. Pierdes 3 niveles y tus tesoros visibles de las manos.', 3, [TreasureKind::ONEHAND,TreasureKind::ONEHAND,TreasureKind::BOTHHANDS], [])
    ...
    p = Prize.new(3, 1)
    b = SpecificBadConsequence.new('Pierdes una mano visible.', 0, [TreasureKind::ONEHAND], [])
    @unusedMonsters << Monster.newCultist('El mal indecible impronunciable', 10, p, b, -2)
    
    #Testigos oculates
    p = Prize.new(2, 1)
    b = NumericBadConsequence.new('Pierdes tus tesoros visibles. Jajaja', 0, 5, 0)
    @unusedMonsters << Monster.newCultist('Testigos oculates', 6, p, b, 2)
    
    
    #El gran Cthulhu
    p = Prize.new(2, 5)
    b = DeathBadConsequence.new('Hoy no es tu dia de suerte. Mueres.')
    @unusedMonsters << Monster.newCultist('El gran Cthulhu', 20, p, b, 4)
    
    
    #Serpiente politico
    p = Prize.new(2, 1)
    b = NumericBadConsequence.new('Tu gobierno te recorta 2 niveles.', 2, 0, 0)
    @unusedMonsters << Monster.newCultist('El mal indecible impronunciable', 8, p, b, -2)
    
    
    #Felpuggoth
    p = Prize.new(1, 1)
    b = SpecificBadConsequence.new('Pierdes tu casco y tu armadura visible. Pierdes tus manos ocultas.', 0, [TreasureKind::HELMET, TreasureKind::ARMOR], [TreasureKind::ONEHAND, TreasureKind::ONEHAND, TreasureKind::BOTHHANDS])
    @unusedMonsters << Monster.newCultist('Felpuggoth', 2, p, b, 5)
    
    #Shoggoth
    p = Prize.new(4, 2)
    b = NumericBadConsequence.new('Pierdes 2 niveles.', 2, 0, 0)
    @unusedMonsters << Monster.newCultist('Shoggoth', 16, p, b, -4)
    
    #Lolitagooth
    p = Prize.new(1, 1)
    b = NumericBadConsequence.new('Pintalabios negro. Pierdes 2 niveles.', 2, 0, 0)
